...
# Question 1: What is the order of longitude and latitude in the setView() function?
  # Answer: Longitude is 151.105006, latitude is -33.8767231
...
l_dk <- leaflet() %>%   # assign the base location to an object
  setView(9.189903, 56.061180, zoom = 6) %>% 
  addTiles()
...
# Add your markers to the existing DanMap object
DanMap <- DanMap %>%
  addMarkers(
    lng = places_clean$Longitude,
    lat = places_clean$Latitude,
    popup = paste(places_clean$Description, "<br>", places_clean$Type)
  )
...
DanMap_clustered <- DanMap %>%
  addMarkers(
    ...
    lat = places_clean$Latitude,
    popup = paste(places_clean$Description, "<br>", places_clean$Type),
    clusterOptions = markerClusterOptions()   # <- this enables clustering
  )
...
# Your brief answer
  # Answer: The map that is not clustered shows each location individually, while the clustered one shows how many are in an area
  # The unclustered is good for exploring each individual one, but can get unreadable/clumsy if there are many locations
  # The clustered is good to gain an overview. You can still see exact location when zooming in, so i prefer this one
...
DanMap_Notes_Classificatins <- DanMap %>%
  addMarkers(
    lng = places_clean$Longitude,
    lat = places_clean$Latitude,
    popup = paste(
      "<b>Description:</b>", places_clean$Description, "<br>",
      "<b>Type:</b>", places_clean$Type, "<br>",
      "<b>Classification:</b>", places_clean$Classification, "<br>",
      "<b>Notes:</b>", places_clean$Notes
    ),
    clusterOptions = markerClusterOptions()   # optional: keep clustering
  )

# updated map
DanMap_Notes_Classificatins
 
# it already displayed the notes and classifications column in the map when pressing on a location
